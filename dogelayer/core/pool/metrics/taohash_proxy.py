...
class ProxyMetrics(BaseMetrics):
    ...
    def get_share_value_fiat(self, coin_price: float, coin_difficulty: float) -> float:
        ...
        # 开发模式下的收益放大倍数，通过环境变量控制，默认为1.0（不放大）
...
def get_metrics_timerange(
    ...
    for hotkey in hotkeys:
        ...
        # 添加详细的worker数据日志
        import logging
        logging.info(f"ProxyMetrics创建 - hotkey: {hotkey}, worker_id: {worker_id}")
        logging.info(f"ProxyMetrics创建 - worker_data原始数据: {worker_data}")
        ...
        logging.info(f"ProxyMetrics创建 - 提取的字段值: hashrate={hashrate_raw}, shares={shares_raw}, share_value={share_value_raw}")
